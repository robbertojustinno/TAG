...
class MultiempresaTests(unittest.TestCase):
    ...
    def test_superadmin_management_and_safe_responses(self):
        ...
        grouped=self.client.get('/superadmin/company-users',headers=headers)
        self.assertEqual(grouped.status_code,200)
        managed=next(item for item in grouped.json() if item['id']==company.json()['id'])
        member=next(item for item in managed['users'] if item['id']==user.json()['id'])
        self.assertEqual(member['role'],'supervisor')
        self.assertTrue(member['membership_active'])
        self.assertNotIn('password_hash',grouped.text)
